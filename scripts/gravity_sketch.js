const CONFIG = {
  gravConst: 0.01,     // Gravitational constant
  numPlanets: 200,    // Number of planets to generate
  numStars: 1,        // Number of stars to generate
  planetMaxMass: 100, // Max planet mass
  planetMinMass: 100, // Min planet mass
  starMaxMass: 10000,  // Max star mass
  starMinMass: 10000   // Min star mass
};
const INPUTS = {}, TEXT = {};
var movers,
    selectedMover = false,
...
function createMovers() {
  ...
  for (var i = 0; i < CONFIG.numStars; i++) {
    ...
    let s = new Mover(random(width), random(height), mass, 80);
    movers.push(s);
  }
  
  for (var i = 0; i < CONFIG.numPlanets; i++) {
    let mass = random(CONFIG.planetMinMass, CONFIG.planetMaxMass);
    let p = new Mover(random(width), random(height), mass, 20);
    movers.push(p);
  }
}
...
function keyPressed() {
  ...
  if (keyCode === 80) {
    let mass = random(CONFIG.planetMinMass, CONFIG.planetMaxMass);
    let p = new Mover(mouseX, mouseY, mass, 20);
    movers.push(p);
  }
  ...
  if (keyCode === 83) {
    let mass = random(CONFIG.starMinMass, CONFIG.starMaxMass);
    let s = new Mover(mouseX, mouseY, mass, 80);
    movers.push(s);
  }
